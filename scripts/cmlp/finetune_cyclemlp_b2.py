...
import numpy as np
def run_finetune_cyclemlp_b2(medical_type, model_type, batch_size, additional_evaluation = False, mean_and_std = False):
    """
...
if __name__ == "__main__":
    ...
    batch_size = 64 
    model_types = ["CycleMLP_B2"]
    medical_types = ['ucsd', 'ori']
    for i in range(len(model_types)):
        ucsd_classifier = run_finetune_cyclemlp_b2(medical_types[0], model_types[i], batch_size, additional_evaluation=medical_types[1], mean_and_std = mean_and_std[medical_types[0]])
        ori_classifier = run_finetune_cyclemlp_b2(medical_types[1], model_types[i], batch_size, additional_evaluation=medical_types[0],mean_and_std = mean_and_std[medical_types[1]])