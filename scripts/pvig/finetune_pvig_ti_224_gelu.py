...
import numpy as np
def run_finetune_pvig_ti_224_gelu(medical_type, model_type, batch_size, additional_evaluation = False, mean_and_std = False):
    """
    Performs fine-tuning of a pvig_ti_224_gelu model on a given medical type and model type.
    ...
    if model_type == "pvig_ti_224_gelu":
...
if __name__ == "__main__":
    ...
    batch_size = 128 
    model_types = ['pvig_ti_224_gelu']
    medical_types = ['ucsd', 'ori']
    ucsd_classifier = run_finetune_pvig_ti_224_gelu(medical_types[0], model_types[0], batch_size, additional_evaluation=medical_types[1], mean_and_std = mean_and_std[medical_types[0]])
    ori_classifier = run_finetune_pvig_ti_224_gelu(medical_types[1], model_types[0], batch_size, additional_evaluation=medical_types[0],mean_and_std = mean_and_std[medical_types[1]])